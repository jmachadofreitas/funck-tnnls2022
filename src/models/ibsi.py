...
class IBSI(ModelMixin, LightningModule):

    def __init__(self, dconfig: DatasetConfig, econfig: ExperimentConfig):
        """
        References:
             Moyer et al. “Invariant Representations without Adversarial Training.”, 2019, ArXiv:1805.09458
        """
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        pred_logits = self._predict(z_mean)
        return {
            "z": z_mean,
            "x": recon_logits,
            "y": pred_logits
    ...
    def _update_metrics(self, x_logits, y_logits, x, y, ctx):
        ...
        if stage not in self.metrics.keys():
            return
    ...
    def _step(self, batch):
        ...
        # loss = (self.alpha + self.beta) * kl_div + self.alpha * recon_loss + pred_loss  # + mmd_loss
...
class SemiIBSI(IBSI):
    ...
    def _labeled_step(self, batch):
        ...
        # loss = (self.alpha + self.beta) * kl_div + self.alpha * recon_loss + pred_loss
        loss = kl_div + self.alpha * recon_loss + self.beta * pred_loss  # + mmd_loss
